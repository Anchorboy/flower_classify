...
class Annotation:
    def __init__(self, image_set, data_path, image_ext=".jpg", with_bbox=True):
        # training set
...
def seperate_train_val_data(x_data, y_data, ratio = 0.8, min_sample_num = 5):
    ...
    if sample_num < min_sample_num:
        ...
        index = np.random.choice(len(x_data), train_num, replace=False)
        other = np.setdiff1d(range(len(x_data)), index, assume_unique=True)
        train_data = x_data[index]
        train_label = y_data[index]
        val_data = x_data[other]
        val_label = y_data[other]
